...
/**
  ...
  */
cv::Mat RGBD2VR::resizeKeepAspectRatio(const cv::Mat &input, const cv::Size &dstSize)
{
    cv::Mat output = Mat::zeros(dstSize, input.type());

    if(input.empty()) {
        ROS_ERROR("Empty image passed to resizeKeepAspectRatio!");
        return output;
    }
    ...
    cv::copyMakeBorder(output, output, top, down, left, right, cv::BORDER_CONSTANT, {0,0,0});
